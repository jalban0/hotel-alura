        ...
        btnbuscar.addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent e) {
                ...
                if (txtBuscar.getText().equals("")) {
                    ...
                    preencherTabelaReservasPorId();
                    preencherTabelaHospedesPorId();
        ...
        btnDeletar.addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent e) {
                ...
                if (listaReservas >= 0) {
                    ...
                    int confirmar = JOptionPane.showConfirmDialog(null, "Excluir os dados?");

                    if(confirmar == JOptionPane.YES_OPTION){
                        String valor = tbReservas.getValueAt(listaReservas, 0).toString();
                        reservaService.deletar(Integer.valueOf(valor));
                        JOptionPane.showMessageDialog(contentPane, "Dados excluídos");
                        limparCampo();
                        preencherReservas();
                        preencherHospedes();
                    }
                    ...
                    if(confirmar == JOptionPane.YES_OPTION){
                        ...
                        hospedesService.deletar(Integer.valueOf(valor));
                        JOptionPane.showMessageDialog(contentPane, "Dados excluídos");
                        limparCampo();
                        preencherReservas();
                        preencherHospedes();
                    ...
                    JOptionPane.showMessageDialog(null, "Lista não selecionada, por favor tente novamente.");
                }
            }
        });
    }
...
//                    String novoValor = calculaValor.converteDados(dataEntrada, dataSaida, valor);
    ...
    private void atualizarHospedes() {
        Optional.ofNullable(modeloHospedes.getValueAt(tbHospedes.getSelectedRow(), tbHospedes.getSelectedColumn()))
                .ifPresentOrElse(lista -> {
                    ...
                    String nome = String.valueOf(modeloHospedes.getValueAt(tbHospedes.getSelectedRow(), 1));
                    String sobreNome = String.valueOf(modeloHospedes.getValueAt(tbHospedes.getSelectedRow(), 2));
                    Date dataNasc = Date.valueOf(modeloHospedes.getValueAt(tbHospedes.getSelectedRow(), 3).toString());
                    String nacionalidade = String.valueOf(modeloHospedes.getValueAt(tbHospedes.getSelectedRow(), 4));
                    String telefone = String.valueOf(modeloHospedes.getValueAt(tbHospedes.getSelectedRow(), 5));
                    Integer reserva_id = Integer.valueOf(modeloHospedes.getValueAt(tbHospedes.getSelectedRow(), 6).toString());
                    this.hospedesService.atualizar(id, nome, sobreNome, dataNasc, nacionalidade, telefone, reserva_id);
                    JOptionPane.showMessageDialog(this, String.format("Atualização realizada com sucesso!"));
                }, () -> JOptionPane.showMessageDialog(this, "Por favor, escolha um registro!"));
    }

    private void preencherTabelaReservasPorId() {
	...
	private void preencherTabelaHospedesPorId() {
		List<Hospedes> hospedes = buscarHospedesPorId();
		try {
			for (Hospedes hospede : hospedes) {
				modeloHospedes.addRow(new Object[]{
						...
						hospede.getTelefone(), hospede.getId()
				});
			}
		} catch (Exception ex) {
			throw ex;
    ...
    private void buscarHospedePorNome() {
        List<Hospedes> hospedes = buscarPorNome();
        try {
            for (Hospedes hospede : hospedes) {
                modeloHospedes.addRow(new Object[]{
                        hospede.getId(), hospede.getNome(), hospede.getSobrenome(),
    ...
    private List<Hospedes> buscarPorNome(){
        List<Hospedes> hospedes = new ArrayList<>();
        if(txtBuscar.getText() != null){
            return this.hospedesService.buscarHospedePorNome(txtBuscar.getText());
        }else{
            return hospedes;
        }
    }
